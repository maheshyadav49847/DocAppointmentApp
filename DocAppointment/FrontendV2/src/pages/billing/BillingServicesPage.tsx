...
export default function BillingServicesPage() {
  ...
  const [formData, setFormData] = useState({
    name: '',
    category: '',
    defaultPrice: 0 as number | string,
    isActive: true
  });
  ...
  const handleOpenModal = (service?: ServiceItem) => {
    ...
    if (service) {
      ...
      setFormData({ name: '', category: '', defaultPrice: 0 as number | string, isActive: true });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasError) {
      try {
        const searchRes = await billingService.getServices(organizationId, 1, 10, formData.name.trim());
        const exists = searchRes.items.some((s: any) => s.name.toLowerCase() === formData.name.trim().toLowerCase() && s.id !== editingService?.id);
        if (exists) {
              ...
              <form onSubmit={handleSubmit} noValidate className="p-6 space-y-4">
                <div>
                  ...
                  <Input 
                    ...
                    className={`w-full px-3 py-2 border rounded-lg outline-none bg-white shadow-sm transition-colors ${errors.name ? 'border-rose-300 focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500' : 'border-slate-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500'}`}
                ...
                <div>
                  ...
                  <Input 
                    ...
                    className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none bg-white shadow-sm transition-colors"
                ...
                <div>
                  ...
                  <Input 
                    type="number" 
                    min="0"
                    value={formData.defaultPrice === null || formData.defaultPrice === undefined ? '' : formData.defaultPrice} 
                    onChange={e => { setFormData({...formData, defaultPrice: e.target.value === '' ? '' : Number(e.target.value)}); setErrors({...errors, price: ''}); }} 
                    className={`w-full px-3 py-2 border rounded-lg outline-none bg-white shadow-sm transition-colors ${errors.price ? 'border-rose-300 focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500' : 'border-slate-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500'}`}
                  />
                  {errors.price && (
                    <p className="text-rose-500 text-xs font-bold mt-1.5 flex items-center gap-1">
                      <AlertCircle className="w-3.5 h-3.5" />
